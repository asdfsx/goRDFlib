...
// --- Parse/Serialize Dispatch (Story 2.5) ---

// Parser parses RDF data into a graph.
type Parser interface {
	Parse(g *Graph, r io.Reader, base string) error
}

// Serializer serializes a graph to a writer.
...
// WithFormat sets the format for parsing/serializing.
func WithFormat(format string) ParseOption {
	return func(c *parseConfig) { c.format = format }
}

// WithParseBase sets the base URI for parsing.
func WithParseBase(base string) ParseOption {
	return func(c *parseConfig) { c.base = base }
}

// Parse reads RDF data from a reader into the graph.
// Ported from: rdflib.graph.Graph.parse
func (g *Graph) Parse(r io.Reader, opts ...ParseOption) error {
	cfg := &parseConfig{format: "turtle"}
	for _, opt := range opts {
		opt(cfg)
	}
...
// WithSerializeFormat sets the serialization format.
func WithSerializeFormat(format string) SerializeOption {
	return func(c *serializeConfig) { c.format = format }
}

// WithSerializeBase sets the base URI for serialization.
func WithSerializeBase(base string) SerializeOption {
	return func(c *serializeConfig) { c.base = base }
}

// Serialize writes the graph to a writer.
// Ported from: rdflib.graph.Graph.serialize
func (g *Graph) Serialize(w io.Writer, opts ...SerializeOption) error {
	cfg := &serializeConfig{format: "turtle"}
	for _, opt := range opts {
		opt(cfg)
	}
	...
	return factory().Serialize(g, w, base)
}
